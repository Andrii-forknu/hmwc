...
public:
    ...
    friend void SaveToTxt(const std::string& filename, const Polynomial& poly);
    friend void LoadFromTxt(const std::string& filename, Polynomial& poly);
   void input(){
        	std::cout << "Enter the size of the polynomial: ";
        	std::cin >> size;
        	delete[] coefficients; // Free old memory
        	coefficients = new double[size];
        	
        	std::cout << "Enter the coefficients (from highest degree to lowest): ";
        	for (int i = 0; i < size; ++i) {
...
void SaveToTxt(const std::string& filename, const Polynomial& poly) {
    std::ofstream outFile(filename);
    if (outFile.is_open()) {
        for (int i = poly.size - 1; i >= 0; --i) {
            outFile << poly.coefficients[i] << std::endl;
        }
        outFile.close();
        std::cout << "Polynomial saved to " << filename << std::endl;
    }
}
void LoadFromTxt(const std::string& filename, Polynomial& poly) {
    ...
    if (inFile.is_open()) {
        ...
        // Update polynomial with new coefficients
        delete[] poly.coefficients;
        poly.size = static_cast<int>(coeffs.size());
        poly.coefficients = new double[poly.size];
        for (int i = 0; i < poly.size; ++i) {
            poly.coefficients[i] = coeffs[poly.size - 1 - i]; // Reverse order
        }
        std::cout << "Polynomial loaded from " << filename << std::endl;
    }
}
int main() {
    ...
    SaveToTxt("poly.txt", p1);
    
    Polynomial p3(1); // Small poly
    LoadFromTxt("poly.txt", p3);
    std::cout << "Loaded P3: ";
    p3.print();
